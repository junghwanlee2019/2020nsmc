...
4) 훈련 데이터셋에 리뷰 및 라벨데이터를 넣어주고 버트에 맞도록 형식 변경을 해준다. 버트는 토크나이저를 통하여 토큰화해준다. 데이터의 길이를 설정해줄 수 있는데 영화 평점 데이터이므로 길이는 128이면 충분하다. 이후 토큰을 숫자 인덱스로 변환해준다.테스트데이터셋도 같은 과정을 거친다. 
